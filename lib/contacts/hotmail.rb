    ...
    def contacts(options = {})
      if connected?
        ...
        while(go) do
          ...
          raw_html = resp.body.split("><").grep(/(?:e|dn)lk[0-9]+/)

          #if live or hotmail
          if raw_html[0].include? "group.aspx"
            raw_html.delete_at 0
          else
            raw_html.delete_at 2
          end

          raw_html.each do |row|
            c_info = row.match(/(e|dn)lk([0-9])+/)

            # Grab info
            case c_info[1]
              when "e" # Email
                email = row.match(/#{email_match_text_beginning}(.*)#{email_match_text_end}/)[1]
                build_contacts << email.sub("%40", "@")
              when "dn" # Name
                build_contacts << row.match(/>(.+)<\/a/)[1]
            end
        ...
        return make_pairs(@contacts)
      end
    end

    def make_pairs(contacts)
      list_pair = []
      pair = []
      i = 0
      (contacts.size/2).times do |t|
        2.times do |e|
          if i == 0
            pair << contacts[e]
          else
